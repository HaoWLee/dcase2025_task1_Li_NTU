...
from datasets.dcase23_split import get_training_set, get_test_set
...
class PLModule(pl.LightningModule):
    def __init__(self, config):
        ...
        #arch="passt_s_swa_p16_128_ap4761",
        arch=config.arch,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if isinstance(y_hat, tuple):  # 👈 加这一行，取 logits
            y_hat = y_hat[0]
        ...
        return loss  # 必须返回 loss，trainer会自动 backward
    ...
    def on_validation_epoch_end(self):
        ...
        logs = {'val/loss': avg_loss, 'val/acc': val_acc}

        for d in self.device_ids:
            dev_loss = torch.stack([x[f"devloss.{d}"] for x in outputs]).sum()
            dev_cnt = torch.stack([x[f"devcnt.{d}"] for x in outputs]).sum()
            dev_corrct = torch.stack([x[f"devn_correct.{d}"] for x in outputs]).sum()

            logs[f"val/loss.{d}"] = dev_loss / dev_cnt
            logs[f"val/acc.{d}"] = dev_corrct / dev_cnt
            logs[f"val/cnt.{d}"] = dev_cnt

            logs[f"acc/{self.device_groups[d]}"] = logs.get(f"acc/{self.device_groups[d]}", 0.) + dev_corrct
            logs[f"count/{self.device_groups[d]}"] = logs.get(f"count/{self.device_groups[d]}", 0.) + dev_cnt
            logs[f"lloss/{self.device_groups[d]}"] = logs.get(f"lloss/{self.device_groups[d]}", 0.) + dev_loss

        for g in set(self.device_groups.values()):
            logs[f"val/acc.{g}"] = logs[f"acc/{g}"] / logs[f"count/{g}"]
            logs[f"val/loss.{g}"] = logs[f"lloss/{g}"] / logs[f"count/{g}"]
...
def train(config):
    ...
    # train dataloader
    train_dl = DataLoader(dataset=get_training_set(config.subset,config.cache_path, config.resample_rate, config.roll, config.dir_prob),
                          worker_init_fn=worker_init_fn,
                          num_workers=config.num_workers,
                          batch_size=config.batch_size,
                          shuffle=True)
    print(f"Train dataset size: {len(train_dl.dataset)} samples")
    # test loader
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default="CPJKU_passt_teacher_training_subset25_ap4761")
    ...
    parser.add_argument('--arch', type=str, default='passt_s_swa_p16_128_ap4761') 
    #parser.add_argument('--arch', type=str, default='passt_s_swa_p16_s16_128_ap473')  # pretrained passt model
