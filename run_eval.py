...
import json
import os
#from datasets.dcase25 import get_training_set, get_test_set, get_eval_set
from datasets.dcase24_librosa import get_training_set, get_test_set, get_eval_set
from helpers.init import worker_init_fn
from models.cp_mobile_clean import get_model
from helpers.utils import mixstyle
from helpers import nessi_2024 as nessi
...
class PLModule(pl.LightningModule):
    ...
    def mel_forward(self, x):
        ...
        """
        old_shape = x.size()
        print("[DEBUG]INPUT SHAPE:",old_shape)
...
def train(config):
    ...
    wandb_logger = WandbLogger(
        ...
        notes="Baseline System for DCASE'24 Task 1 speed.",
        tags=["DCASE24"],
        config=config,
        name=unique_name
    )
...
def evaluate(config):
    ...
    import pandas as pd
    import torch.nn.functional as F
    from datasets.dcase24_librosa import dataset_config
    ...
    state_dict = torch.load(config.ckpt_path, map_location="cpu")["state_dict"]
    ...
    print("[Info] Test set size:",len(test_dataset))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--kd_lambda', type=float, default=0.005)
