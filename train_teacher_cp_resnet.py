...
class PLModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        return loss  # 必须返回 loss，trainer会自动backward
...
def train(config):
    ...
    train_dl = DataLoader(dataset=get_training_set(config.subset,config.cache_path, config.resample_rate, config.roll,
                          ...
                          shuffle=True)
    dataset=get_training_set(config.subset,config.cache_path, config.resample_rate, config.roll,config.dir_prob)                      
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cache_path', type=str, default=os.path.join("datasets", "cpath_25p"))
