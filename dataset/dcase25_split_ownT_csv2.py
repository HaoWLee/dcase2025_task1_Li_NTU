...
dataset_dir_eval = "/home/lihaowen/dcase_evaluation_set_2025"
# dataset_dir = None
...
dataset_config = {
    ...
    "eval_dir": os.path.join(dataset_dir_eval),
    "eval_fold_csv": os.path.join(dataset_dir_eval, "TAU-urban-acoustic-scenes-2025-mobile-evaluation",
    ...
    #"logits_file": os.path.join("ensemble/", "ensemble_logits_2T_resnet.csv")
    #"logits_file": os.path.join("ensemble/", "ensemble_logits_9T.csv")
    #"logits_file": os.path.join("ensemble/", "ensemble_logits_8T_332.csv")
}
...
def get_test_set(cache_path=None, resample_rate=44100):
    os.makedirs(dataset_config['split_path'], exist_ok=True)
...
class BasicDCASE24EvalDataset(TorchDataset):
    ...
    def __getitem__(self, index):
        sig = librosa.load(os.path.join(self.eval_dir, self.files[index]), sr=32000, mono=True)
        sig = torch.from_numpy(sig[np.newaxis])
